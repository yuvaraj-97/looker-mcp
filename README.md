...
To prevent massive token bloat and API costs, this MCP server uses a **Dynamic Dispatch Pattern**. Rather than exposing all 483 endpoints as individual tools (which would overwhelm LLM context windows), it exposes exactly **3 highly-efficient exploration tools**:

1. `list_looker_groups()`: Returns the top-level API categories (e.g., `Dashboards`, `Looks`, `Users`).
2. `list_group_endpoints(group_name)`: Returns the exact endpoint names and parameter schemas for a specific group.
3. `call_looker_endpoint(endpoint_name, arguments)`: Executes any of the 483 endpoints using a JSON dictionary of arguments.

This guarantees 100% full coverage of the Looker API while consuming virtually zero token overhead during idle prompts!
